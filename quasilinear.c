/*
    ...
    Или второй вариант для каждого слова вычисляется чило Фибоначчи
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
char **loadfile(FILE *fd) {
    ...
    while (fscanf(fd, "%s", str) == 1 ) {
        ...
        fib(strlen(pp[n])*n);                       // трудоёмкость по операции вычисления чисела Фибоначи быстрым возведением матрицы в степень имеет вид T=log(N)
